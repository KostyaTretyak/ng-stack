...
Тут `:postId` у властивості `path` вказує нам на те, що цей ключ повинен використовуватись в якості "primary key" для елементів, що повертаються функцією `dataCallback`.
